...
public class JWTProvider {
    ...
    private static final int HOURS_VALIDITY = 30;

    private static final int DAYS_VALIDITY = 12;
    ...
    public Authentication getAuthentication(String token) {
        String login = retrieveClaims(token, ACCESS).get("login").asString();
        UserDetails userDetails = userDetailsService.loadUserByUsername(login);
        return new UsernamePasswordAuthenticationToken(userDetails, null, userDetails.getAuthorities());
    }

    public String generateToken(JWTType type, String login) {
        var now = ZonedDateTime.now().toInstant();

        Instant expiration = type == REFRESH ?
                now.plus(DAYS_VALIDITY, DAYS) :
                now.plus(HOURS_VALIDITY, HOURS);
